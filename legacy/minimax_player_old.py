...
def minimax(board, move_list, player, tree_depth):
    ...
    if player == 1:
        ...
        for move in move_list:
            ...
            new_v = min_value(new_board, alpha, beta, 0 - player, tree_depth)
            if new_v > v:
                v = new_v
                alpha = v
                optimal_move = move
        ...
        for move in move_list:
            ...
            x1, y1, x2, y2 = move
            new_board.make_move(((x1,y1),( x2,y2)))
            new_v = max_value(new_board, alpha, beta, 0 - player, tree_depth)
            if new_v < v:
                v = new_v
                beta = v
                optimal_move = move
...
def max_value(board, alpha, beta, player, depth):
    ...
    for move in move_list:
        ...
        x1, y1, x2, y2 = move
        new_board.make_move(((x1,y1),( x2,y2)))
        new_v = min_value(new_board, alpha, beta, 0 - player, depth - 1)
        v = max(v, new_v) 
...
def min_value(board, alpha, beta, player, depth):
    if board.finished() or depth == 0:
        return evaluate(board.board)
    v = INF
    start, end = board.moves[-1]
    ...
    for move in move_list:
        ...
        x1, y1, x2, y2 = move
        new_board.make_move(((x1,y1),( x2,y2)))
        new_v = max_value(new_board, alpha, beta, 0 - player, depth - 1)
        v = min(v, new_v)
        beta = min(new_v, beta)
        if beta < alpha:
            # print(f"Break New: {alpha} {beta}")
